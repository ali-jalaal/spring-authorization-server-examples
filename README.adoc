...
This projects provides custom configuration for various features implemented by Spring Authorization Server.

[[run-examples]]
=== Run Examples
Modules can be run in two ways:

1. Locally:
* Run Authorization Server -> `./gradlew -b auth-server/build.gradle.kts bootLocalRun`
* Run Client -> `./gradlew -b client/build.gradle.kts bootLocalRun`
* Run Resource Server -> `./gradlew -b resource-server/build.gradle.kts bootLocalRun`

2. Docker Compose (Needs docker-compose be installed locally):
* Assemble modules (build without running tests) -> `./gradlew assembleAll`
* Run docker compose: `./gradlew composeUp`
* To stop docker compose containers: `./gradlew composeDownForced`
Please note that
When modules are running either way:
